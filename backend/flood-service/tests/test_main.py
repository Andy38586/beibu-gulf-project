...
垂直基准统一（2026-08-29）：online 入口把前端理论深度基准面水位减
datum_offset 换算 EGM96 后查表/演算，响应 level 回显理论档位。测试用
_fixed_datum 固定偏移 2.5（与 waterLevel.json baseLevels.msl 同口径），
假表键/缓存键断言均为换算后（EGM96）口径。
"""
...
def _fixed_datum(monkeypatch):
    """垂直基准统一（2026-08-29）：固定偏移 2.5，测试口径确定、不依赖数据文件"""
    monkeypatch.setattr(main_mod, "datum_offset", lambda: 2.5)
...
def test_same_level_second_request_cache_hit_no_500(client, monkeypatch):
    ...
    assert main_mod._cached_level[1.0] is not None  # 缓存键为 EGM96 口径（3.5−2.5），move_to_end 后键仍在
...
def test_lookup_miss_falls_back_to_lru_compute(client, monkeypatch):
    ...
    assert 4.7 in main_mod._cached_level  # miss 结果已入 LRU 缓存（EGM96 键）
...
def test_below_msl_returns_empty_without_compute(client, monkeypatch):
    """垂直基准统一（2026-08-29）：理论水位低于平均海平面（EGM96 键 <0）→
    空淹没响应、零演算、不触碰 DEM（cut 版 DEM 缺失时也不致 500）"""
    calls: list[float] = []
    monkeypatch.setattr(
        main_mod, "run_online_flood", lambda level: (calls.append(level), _fake_run(level))[1]
    )
