...
describe('floodService.assessDisaster — 脏数据防御（B-9 修复后守护）', () => {
  it('B-9：elevation=null 应排除（不隐式判 0 假阳性）', () => {
    const r = assessDisaster([{ id: 'f1', name: 'A', elevation: null, value: 100, damageRate: 0.5 }], 5, floodZone)
  ...
  it('B-9：value 缺失应计 loss=0、totalLoss 有限', () => {
    const r = assessDisaster([{ id: 'f1', name: 'A', elevation: 2, value: undefined, damageRate: 0.5 }], 5, floodZone)
