...
//
// 空间算子已下沉 PostGIS：凡涉及 buffer/union/intersect/点面判定的用例需真库，
// 按 favorites/plans 同款口径以 V3_INTEGRATION_DB 控制（无库环境跳过，避免 ECONNREFUSED 噪音）。
// 纯参数校验/清洗/加权排名的用例不碰数据库，保持无库可跑。
...
describe.skipIf(!withDb)('选址空间算子（真库 PostGIS）— 覆盖 / 求交 / 筛选', () => {
  beforeAll(() => {
    db = new DbService()
  ...
  it('buildTypeCoverage：无有效点返回 null；有效点返回带 geometry 的 Feature', async () => {
    await expect(buildTypeCoverage(spatial, [], 3)).resolves.toBeNull()
    await expect(buildTypeCoverage(spatial, null, 3)).resolves.toBeNull()
    const cov = await buildTypeCoverage(spatial, [{ lng: 108.6, lat: 21.85 }], 3)
    expect(cov).toBeTruthy()
    expect(cov?.geometry).toBeTruthy()
    // 与旧 turf 输出同构：GeoJSON Feature + properties 空对象（前端按 Feature 消费）
    expect(cov).toMatchObject({ type: 'Feature', properties: {} })
  })

  it('intersectCoverages：无有效覆盖 → { area:null, failKey:null }', async () => {
    await expect(
      intersectCoverages(spatial, [null, undefined] as never[], ['hospital', 'school'])
    ).resolves.toEqual({ area: null, failKey: null })
  })

  it('intersectCoverages：不相交 → area=null 且 failKey 指向断裂类型', async () => {
    const a = await buildTypeCoverage(spatial, [{ lng: 108.6, lat: 21.85 }], 2)
    const b = await buildTypeCoverage(spatial, [{ lng: 114.5, lat: 24.5 }], 2)
    await expect(intersectCoverages(spatial, [a, b], ['hospital', 'school'])).resolves.toEqual({
      area: null,
      failKey: 'school',
    })
  })

  it('filterMatchedXiaoqu：空数据返回 []；无索引时全量精筛', async () => {
    await expect(
      filterMatchedXiaoqu(spatial, [], {
        type: 'Feature',
        properties: {},
        geometry: { type: 'Polygon', coordinates: [] },
      })
    ).resolves.toEqual([])
    const area = await buildTypeCoverage(spatial, [{ lng: 108.6, lat: 21.85 }], 3)
    const matched = await filterMatchedXiaoqu(
      spatial,
      [
...
describe.skipIf(!withDb)('SiteAnalysisService.runSiteAnalysis — 选址流水线（R-14，真库）', () => {
  ...
  it('正常输入 → 返回 error=null 且结构完整（coverage/matchedXiaoqu/facilityPoi）', async () => {
    const result = await run({ selectedKeys, typeSettings, facilityData, xiaoquData })
  ...
  it('facilityPoi 返回各选中类型参与评分的全部合法 POI（含覆盖区外的点）', async () => {
    const result = await run({ selectedKeys, typeSettings, facilityData, xiaoquData })
    ...
    const wide = await run({ selectedKeys, typeSettings, facilityData: wideData, xiaoquData })
    expect(wide.facilityPoi.hospital).toHaveLength(2)
  })

  it('facilityPoi 过滤脏数据：越界/[0,0]/重复坐标不参与（与覆盖计算入参同源）', async () => {
    const dirtyData = {
      hospital: [
  ...
  it('全部类型覆盖数据不可用 → emptyReason 走 failKey=null 专属文案', async () => {
    const result = await run({
  ...
  it('排名按 score 降序且截断至 TOP_N=10', async () => {
