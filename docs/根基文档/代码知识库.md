...
> ⚠️ **基线声明（2026-09-20）**：本文按 **2026-09-09** 的代码写成，此后仓库又走了 155 笔提交（含 Express 与 FastAPI 退役、v4 异步任务域上线、PostGIS 下沉）。**读法**：现状一律以代码 + 01/02/03 + `docs/README.md` 为准；本文只作导航与背景，**文末「现状勘误（2026-09-20）」所列条目全部作废**，其余段落也可能仍滞后（未逐段复核），引用前先 `grep` 验证路径与数量。
...
- `spatialIndex.ts`：**前端版** rbush 矩形视口裁剪（与后端 `modules/site-analysis/services/spatial-index.ts` 不同）。
...
| modules/flood/controllers/flood.controller.ts | 洪涝数据/分析接口；`deriveRiskLevel()`（6 档语义）、`lookupFloodZone()`（查 251 档表） |
| plansController.js                            | 方案 CRUD + 小区保存/移除；用户归属校验 + 名称唯一校验                                 |

## 5. 服务层（services/）——业务计算

### 5.1 modules/site-analysis/services/site-analysis.service.ts（选址分析）
...
### 5.2 modules/site-analysis/services/scoring.ts（选址评分核心）
...
### 5.3 modules/forecast/services/forecast.service.ts（预测）
...
### 5.4 modules/forecast/services/forecast-engine.ts（预测引擎）
...
| common/cache/read-cache.ts                      | 读文件缓存工厂（TTL 5min + LRU/FIFO 淘汰 + 容量上限）                                                         |
| infra/files/data-files.service.ts               | 读 `backend/data/` JSON 统一入口（带缓存）；测试钩子 `_clearCacheForTest`                                     |
| fileStore.js                                    | `createFileStore`：文件存储工厂（读缓存 + 写锁 + 原子写入 tmp+rename）                                        |
| logger.js                                       | 分级日志 + audit                                                                                              |
| modules/site-analysis/services/spatial-index.ts | **后端版** RBush 多边形覆盖查询（`createSpatialIndex` + `queryByPolygon`，BBox 粗筛 + booleanPointInPolygon） |
...
## 10. 2026-09-12 收口回写（审查波欠账补登）

- **认证态**：`isAuthenticated` 已上移 `frontend/src/shared/composables/useAuth.ts`，判据为
  `token !== '' || user !== null` 双源（原 useApiRequest.isAuthenticated 已删除）；恢复期判定
  `isAuthRestoreDone()`（restorePromise 在途单例 + authRestored）；登出不清恢复标志。
- **水位常量**：`backend/src/common/constants/flood.constants.ts MAX_WATER_LEVEL = 25`（后端全域）；
  `frontend/src/shared/constants/flood.ts` 同名常量（守卫对齐用）+ `FLOOD_DISPLAY_MAX_WATER_LEVEL = 10`
  （前端展示域）；漂移由 `tools/v3-guard/constants-audit.mjs` 断言（挂 guard:v3）。
- **基准口径**：浸没全域为 EGM96 海平面基准（前端展示子集 0-10m；详见
  `docs/问题副本-2026-09-12-审查波归档与覆盖对账.md` §六）。
- **DB 池**：`infra/db/db.service.ts` 构造器挂 `pool.on('error')`——DB 重启/闪断不再击穿进程
  （每请求级致命错误仍由 `isFatalDbError` 上抛，两层分层）。
- 本文 §7/§8 中 algorithm-service、`node server/index.js`、`dev:flood` 等旧口径属历史叙述，
  以 `v3-施工手册.md` 与根 `package.json` 为准（全量漂移清理另行排期）。

---

## 现状勘误（2026-09-20）｜以下条目已按现码逐条核实，**正文原述一律作废**

| #   | 正文原述（作废物）                                                               | 现状                                                                                                                                               | 现证据                                                                                                               |
| --- | -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| 1   | 后端在 `backend/nest/`；须 `node server/index.js` / `backend/index.js` 启动      | Nest 在 `backend/src/`，入口 `main.ts`，全局前缀 `nest-api`                                                                                        | `backend/src/main.ts`                                                                                                |
| 2   | `RouteAnalysisPage` 为"占位未实现"                                               | 已实现：航线域 controller/service + `route` 域 pgRouting 查询 + manifest 带 taskPanel                                                              | `backend/src/modules/route/`、`frontend/src/business/manifest.ts`                                                    |
| 3   | online 模式走 FastAPI `/flood-online/*`；`VITE_DATA_SOURCE` 注入                 | 前端已收敛为**单模式**：`dataSource`/`setDataSource` 与多档缓存已删；`/flood-online` 代理与 nginx location 已删；该 env 标注废弃且**代码零读取点** | `frontend/src/services/adapters/floodAdapter.ts`、`frontend/src/main.ts`、`nginx.conf:2,68`、`frontend/.env.example` |
| 4   | 浸没演算由 FastAPI scipy 8 连通在线完成                                          | 在线只查 PostGIS `flood_levels`（251 档 0.1m）；重演算在离线管线                                                                                   | `backend/src/modules/flood/repositories/flood.repository.ts:10,38`                                                   |
| 5   | 选址数据在 `backend/data/site-selection/`；`users/plans/favorites.json` 为文件源 | 该目录不存在，均入库 PostGIS；`flood_levels.json.gz` 亦无前端/后端消费者                                                                           | `backend/src/modules/site-analysis/repositories/`、`.gitignore`                                                      |
| 6   | 依赖守护规则 11 条；后端有 `utils/` 目录                                         | 现 18 条（含 5 条 Nest 分层规则）；`backend/src/` 无 `utils/`，空间算子在 `infra/db/spatial.repository.ts`                                         | `.dependency-cruiser.cjs`                                                                                            |
| 7   | 前端 4 个 store；`CesiumRenderer.ts` 1857 行；3 个渲染器测试                     | 入库 4 个 + `taskStore`（v4，在途未提交时按 `git ls-files` 口径）；渲染器已 2144 行、renderers 测试 9 个                                           | `frontend/src/stores/`、`wc -l frontend/src/core/map/renderers/CesiumRenderer.ts`                                    |
| 8   | Compose 四服务 / CI 5 job                                                        | compose 3 服务（app/nest/postgis）；CI **8 job**（名见根 `README.md`）                                                                             | `docker-compose.yml`、`.github/workflows/ci.yml`                                                                     |
| 9   | `docs/问题副本-2026-09-12-审查波归档与覆盖对账.md` 可查                          | 该文件已不在仓库（内容并入 `docs/audits/2026-09-12-*`）                                                                                            | `ls docs/audits/`                                                                                                    |
