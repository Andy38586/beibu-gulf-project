...
> 与审查体系关系：见 `审查体系约定.md`（一级质量属性 #6 WebGIS 工程正确性）
...
本文档是「WebGIS 专项审查标准库」，是审查体系 8 专项之一（见 `审查体系约定.md` §3）。
...
- **输入**：本文档（可按「部分」切片，仅加载本子 agent 负责的部分）+ 对应代码范围 + 设计文档对应章节（见 `审查体系约定.md` §4「标准输入资产」，本专项映射：01 三.2/四；02 5.1-5.3；03 1.2）。
- **输出**：发现清单（每条含：指标编号 / 证据 file:line / 风险等级 / 整改建议）→ 写入 `docs/audits/<日期>-<批次名>/专项4-WebGIS审查-问题副本.md`（产出统一规范见 `审查体系约定.md` §4；副本规则见 docs/待解决问题.md 头部），经用户裁决转正主台账；执行记录存同批次目录 `专项4-WebGIS审查-执行记录.md`（模板见 §0.7）。
- **子 agent 拆分**：按「第 X 部分」切分（拆分规则见 `审查体系约定.md` §4：① 1 部分=1 子 agent；② 相邻部分证据重叠>50% 可合并；③ 单专项上限 6；④ 禁止二次拆分）。
...
> 产出位置与命名见 `审查体系约定.md` §4「产出统一规范」；发现条目禁止使用草稿编号。
...
### 指标 7.6：地图要素气泡（悬浮/钉住）生命周期与互斥

**指标名称**：地图要素气泡生命周期与互斥
**检查目标**：确认地图 POI 信息气泡（Overlay/浮层类）的打开/关闭/切换状态机正确：悬浮与点击双模式语义清晰、同一时刻仅一个、锚点随地图移动精确跟随、引擎切换正确收起
**为什么需要检查**：气泡是地图交互的高频 Surface，状态机破损的典型症状：悬浮幽灵气泡不消失、钉住气泡随地图漂移错位、双气泡并存、切引擎后气泡残留在错误坐标系——全部是用户一眼可见的交互破损
**检查范围**：气泡组件、锚点定位实现（OL Overlay / Cesium 逐帧坐标换算）、宿主地图事件（pointermove / click）、引擎切换路径
**检查方法**：

1. 检查气泡定位实现：2D 应使用引擎原生锚点机制（OL Overlay，随地图移动自动跟随）；3D 若实现须逐帧坐标换算（postRender + 屏幕坐标投影）并处理视锥外隐藏
2. 检查双模式状态机：悬浮即显、移开即隐；点击钉住、再点同一要素关闭、点其他要素切换；钉住态悬浮不抢当前气泡
3. 检查单实例互斥：全局同一时刻最多一个气泡（状态源唯一，钉住优先于悬浮，非数组/多 Overlay 并存）
4. 检查引擎切换与卸载：切 3D（无气泡能力）时气泡收起；Overlay 随地图销毁释放，不悬挂
5. 检查锚点坐标来源：钉住/悬浮锚点取要素自身坐标（非光标点击处），保证跟随精确
6. 检查事件配对与守卫：pointermove/click 注册注销配对；地图拖拽中不触发悬浮命中
   **需要查看**：core/map/components/ 气泡组件、core/map/UnifiedMap.vue（气泡状态机）、core/map/renderers/OLRenderer.ts（Overlay 与悬停命中）、types/renderer.ts（hover 事件与气泡能力声明）
   **正常标准**：

- 悬浮即显移开即隐；点击钉住跟随；同要素再点关闭；异要素切换；空白点击关闭
- 同一时刻仅一个气泡（状态源唯一）
- 锚点取要素坐标，平移缩放跟随无漂移；引擎切换/容器卸载气泡收起且 Overlay 释放
- 事件注册注销配对；拖拽中无悬浮误触发
  **异常情况**：
- 悬浮气泡移开不消失；钉住气泡在地图移动后与要素错位
- 双气泡并存（钉住一个悬浮又弹一个）
- 切 3D 后 2D 气泡残留
- Overlay 未随地图销毁（内存泄漏）
- 光标移动高频重命中（无拖拽守卫）
  **风险等级**：P2
  **整改方向**：状态源收敛为单实例；锚点走引擎原生跟随机制；引擎切换统一收起；事件配对注销
  **验收标准**：五条交互路径（悬浮/移开/点击钉住/再点关闭/异要素切换）行为正确；单实例；跟随无漂移；引擎切换收起
...
### 指标 8.5：业务模块图层 key 命名空间

**指标名称**：业务模块图层 key 命名空间
**检查目标**：确认每个业务模块的图层 key 带模块前缀，跨模块不撞名、可按模块批量操作。
**为什么需要检查**：BLM 的 registry 是可见性唯一权威，key 全局唯一。模块各自起短名（如 `path`、`analysis`）会在批量显隐、登出重置、引擎切换重建时互相覆盖——表现为"切个页面图层就没了"这类难复现的问题，排查成本极高。
**检查范围**：`frontend/src/business/*/`、`core/map` 的 BLM 注册与更新点、business manifest。
**检查方法**：

1. `grep -rn "register(\|updateData(" frontend/src` 抽取 key 字面量，确认形如 `route-path` 的模块前缀。
2. 确认前缀与模块名一一对应，无两个模块共用前缀。
3. 触发登出批量重置（manifest reset 声明），断言 registry 中该模块 key 全部撤净。
   **需要查看**：各业务 composable 的图层注册处、manifest、BLM 实现。
   **正常标准**：key 均为 `<模块>-<语义>`；无裸 key；批量重置后 registry 无该模块残留。
   **异常情况**：可 grep 的模式——`register('path')` 之类无前缀 key；两个模块用同一 key；key 前缀与模块名不一致；登出后 registry 仍留有该模块 key。
   **风险等级**：P2
   **整改方向**：key 统一加模块前缀；新模块在 manifest 声明 reset；补 key 唯一性与重置无残留测试。
   **验收标准**：□ 全量 key 带模块前缀 □ 无跨模块重名 □ 登出重置无残留有测试

### 指标 8.6：双引擎能力检查与图层注册撤除对称

**指标名称**：双引擎能力检查与图层注册撤除对称
**检查目标**：确认单引擎专有能力调用前做能力检查、不留空实现；确认图层在 2D/3D 的注册与撤除成对。
**为什么需要检查**：02 §5.1 要求能力缺失由调用方检查跳过、禁止空实现 stub。路径线这类新图层若只在 3D 注册而 2D 忘了撤除（或反之），引擎切换后会残留幽灵图层，或产生静默报错——两引擎行为不对称是双引擎架构最容易腐化的地方。
**检查范围**：`core/map/renderers/`（OL 与 Cesium 实现）、业务模块的图层注册与卸载、BLM `reapplyAll`。
**检查方法**：

1. 确认能力接口（如 3D 水面能力）只在具备该能力的引擎实现，调用方先做能力检查再调用。
2. 同一图层 key 在 2D、3D 各跑一遍：注册 → 撤除 → 切换引擎，断言 registry 与控制台都干净。
3. 构造单层失败场景，确认 `reapplyAll` 单层失败不拖垮整批。
   **需要查看**：Renderer 接口与两个实现、BLM、业务模块注册/卸载代码。
   **正常标准**：能力检查在调用点；两引擎下注册与撤除对称；切换后无残留、无静默报错。
   **异常情况**：可 grep 的模式——引擎内留空方法体当 stub（静默 return 而非抛出/不检查）；调用方直接调专有能力不做能力判断；切引擎后 registry 出现孤儿 key；控制台出现图层不存在类静默告警。
   **风险等级**：P2
   **整改方向**：补能力检查；统一注册/撤除入口；补切换后 registry 断言测试。
   **验收标准**：□ 无空实现 stub □ 两引擎注册撤除对称有测试 □ 切换后 registry 干净
