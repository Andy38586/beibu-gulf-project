...
const sql = []
sql.push('BEGIN;')
sql.push('TRUNCATE users, plans, ports, poi_facilities, xiaoqu, flood_facilities, data_archive RESTART IDENTITY CASCADE;')
...
const archiveFiles = [
  'ports.json',
  ...['index', 'cargo', 'container', 'throughput', 'throughput_model', 'traffic', 'berth'].map((n) => `forecast/${n}.json`),
  ...['xiaoqu', 'qz_hospital', 'qz_primary_school', 'qz_middle_school', 'qz_park', 'qz_bus_station', 'qz_mall_and_supermarket'].map((n) => `site-selection/${n}.json`),
  ...['facilityPoints', 'floodArea', 'floodStatistics', 'waterLevel', 'water-area', 'terrainProfile'].map((n) => `flood/${n}.json`),
]
for (const rel of archiveFiles) {
  ...
  sql.push(`INSERT INTO data_archive (name, payload, sha256) VALUES (${esc(rel)}, '${payload}'::jsonb, '${sha}');`)
}
